...
#
__version__='$Id: 0de6fe513fe13f48f23378f4d475aa5d82222fdc $'
#
# Distributed under the terms of the MIT license.
#

import sys, re, time, codecs, string
...
class XmlDumpReplacePageGenerator:
    ...
    """
    def __init__(self, xmlFilename, xmlStart, prereplacements, replacements, postreplacements, exceptions, saves):
        self.xmlFilename = xmlFilename
        self.prereplacements = prereplacements
        self.postreplacements = postreplacements
...
class ReplaceRobot:
    ...
    """
    def __init__(self, generator, prereplacements, replacements, postreplacements, exceptions={}, saves=[],
                 acceptall=False, allowoverlap=False, recursive=False,
                 addedCat=None, sleep=None, editSummary='', articles=None,
                 exctitles=None, noOnceRules=True):
        """
        Arguments:
...
#Hack replace-es
    ...
    def doReplacements(self, original_text):
        ...
        the given text.
        """
        new_text,sign,save_list = save_pat(original_text,self.saves)
        pre_text = new_text
        #print(new_text)
#        new_text = original_text
        ...
        while True:
          for tup in self.replacements:
              ...
              if self.sleep is not None:
                  time.sleep(self.sleep)
              repl_text = pywikibot.replaceExcept(repl_text, old, new,
                                                exceptions,
                                                allowoverlap=self.allowoverlap,
                                                replaceOnce=tup[2:]!=())
          if repl_text == new_text:
              break
          else:
              new_text = repl_text
          # if self.noOnceRules:
        ...
        new_text = restore_pats(new_text,sign,save_list)
        return new_text
    ...
    def run(self):
        ...
        for page in self.generator:
            ...
            new_text = original_text
#Hack replece-es
            fixes.reset_file(original_text)
            while True:
...
def main(*args):
    ...
    replacements = []
    # List of preserved places in articles.
    saves = []
    ...
    if (len(commandline_replacements) % 2):
        ...
        try:
            prereplacements = fix['prereplacements']
            replacements = fix['replacements']
            postreplacements = fix['postreplacements']
    ...
    for i in range(len(saves)):
        try:
          saves[i] = re.compile(saves[i], flags)
        except:
          print(saves[i])
          raise
    ...
    for i in range(len(replacements)):
        if len(replacements[i])>2:
            noOnceRules = False
        old, new = replacements[i][:2]
        #print old
        if not regex:
            old = re.escape(old)
        try:
          ...
          print(old)
          raise
        replacements[i] = (oldR, new)+replacements[i][2:]


    # Pre-compile all regular expressions here to save time later
    for i in range(len(prereplacements)):
        old, new = prereplacements[i]
        #print old
        if not regex:
            old = re.escape(old)
        try:
        ...
        prereplacements[i] = oldR, new

    # Pre-compile all regular expressions here to save time later
    for i in range(len(postreplacements)):
        old, new = postreplacements[i]
        #print old
        if not regex:
            old = re.escape(old)
        try:
    ...
    if xmlFilename:
        ...
        gen = XmlDumpReplacePageGenerator(xmlFilename, xmlStart,
                       prereplacements, replacements, 
                       postreplacements, exceptions, saves)
    elif useSql:
    ...
    if excoutfilename:
        try:
            ...
            pywikibot.output("%s cannot be opened for writing." %
                             excoutfilename)
            return
    bot = ReplaceRobot(preloadingGen, prereplacements, 
                  replacements, postreplacements, exceptions, saves, acceptall,
                  allowoverlap, recursive, add_cat, sleep, editSummary,
                  titlefile, excoutfile, noOnceRules=noOnceRules)
    try:
        bot.run()
    finally:
...
def save_pat(text,pats):
    save_list = []
    #tlpat = re.compile(ur'<timeline>', re.UNICODE)
    sign = ur'_XZXY_'
    spat = re.compile(sign)
    while spat.search(text)!=None:
        sign = sign+sign
        spat = re.compile(sign)
    for pat in pats :
        m = pat.search(text)
        while m!=None:
            save_list.append(m.group())
            text = pat.sub(sign+'<%s>' % len(save_list)+sign,text,1)
            m = pat.search(text)
    #print save_list
    return text,sign,save_list

def restore_pats(text,sign,save_list):
    n = len(save_list)
    while n>0:
        text = string.replace(text,sign+'<%s>' % n+sign,save_list.pop())
        n = n-1
    return text
